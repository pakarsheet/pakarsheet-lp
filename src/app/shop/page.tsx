...
import { Search, Sparkles, ArrowRight, LayoutGrid, List } from "lucide-react";
import Image from "next/image";
import Link from "next/link";
import { useState } from "react";

const categories = ["Semua", "Keuangan", "Marketing", "Inventory", "HR & Admin"];
      ...
      <main className="flex-1 container mx-auto px-4 md:px-6 pt-32 pb-24 relative z-10">
        ...
        <div className="max-w-6xl mx-auto mb-12 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <motion.div 
            ...
            className="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-4 md:pb-0 hide-scrollbar"
          >
            {categories.map((cat) => (
              <button
                key={cat}
              ...
              >
                <Link href={`/shop/${product.id}`} className="block group h-full">
                  <div className="h-full relative flex flex-col rounded-[32px] border border-white/10 bg-[#0a0a0a] overflow-hidden hover:border-white/20 hover:bg-[#0f0f0f] transition-all duration-500">
                    ...
                    {/* Inner Image Container (Floating look) */}
                    ...
                    <div className="p-8 flex flex-col flex-1 relative z-10">
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <span className="text-[11px] font-semibold text-neutral-400 uppercase tracking-widest">Tersedia</span>
                      </div>
                      
                      <h3 className="text-2xl font-semibold text-white/90 mb-3 tracking-tight group-hover:text-white transition-colors">{product.name}</h3>
                      <p className="text-neutral-500 text-sm mb-8 flex-1 leading-relaxed line-clamp-2">{product.description}</p>
